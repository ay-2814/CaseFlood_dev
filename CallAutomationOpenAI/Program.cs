...
//Assistant client to be used with v2 API for File retrieval
AssistantsClient asst_client = new AssistantsClient(new Uri(endpoint), new AzureKeyCredential(key));
...
async Task NewCall(EventGridEvent eventGridEvent  ) {
        ...
        string clientCategory = builderierer["ClientCategory"];
        string calllang="en-US";
        bool can_transfer = bool.Parse(builderierer["Transfer"]);
        //Set Specific Voice
        ...
        Your law firm has these employees: {employeenames}.
        Your firm practices: {relateddoings}.
        Important Guidelines:
        Very important: Make sure to wait for the caller to finish speaking and DO NOT INTERRUPT THE CALLER!!
        Very important: Do not repeat yourself!! Do not keep saying I'm sorry to hear that. Do not say thank you for the information twice! Saying it once is enough. Do not repeat questions if they have been answered.
        
        Other guidelines:
        Keep responses short, and concise.
        Be empathetic, positive, supportive and friendly to the caller in your responses.
        ...
        If yes, identify immediately whether the caller is a new lead, existing client, judge or oppposing counsel. 
        If its a new lead, follow the following 
        ...
        int response_num = 0;
        string clientCategoryDetermined = "";
        ...
        helloPrompt+=$", thank you for calling {companyname}, this is Luna, their AI receptionist. I'm here to help! Are you a new client?";
        ...
        var chatCompletionsOptions = new ChatCompletionsOptions()
            {
                ...
                PresencePenalty = 1.4f
                // NucleusSamplingFactor = 0.25f
            };
            
        // ChatClient chatClient = azureClient.GetChatClient("lil-MissKind-Luna");
        // ChatCompletion completion = chatClient.CompleteChat(
        //     new SystemChatMessage(answerPromptSystemTemplate),
        //     new SystemChatMessage(helloPrompt),
        
        // );
        
    
       /* 
       This block contains codes pertaning to implementing the chat assistant using v2 
       which has file reading capabilities supported
       When implemented, the chatCompletion options for the rest of the snippets will also need to be changed

        //store file path in a var
        var filepath = "/Users/ayushmansrivastava/Documents/CaseFlood/Dev/CallAutomationOpenAI/CaseFlood Goldendoc.pdf";
        

        //Uploading the file to assistant
        FileStream fileStream = new FileStream(filepath, FileMode.Open, FileAccess.Read);
        
        Response<OpenAIFile> uploadAssistantFileResponse = await asst_client.UploadFileAsync(
            data: fileStream,
            purpose: OpenAIFilePurpose.Assistants
            //filename: "CaseFlood_Goldendoc.txt"
        );
        ...
        //Once uploaded, the file ID can then be provided to an assistant upon creation. 
        //Note that file IDs will only be used if an appropriate tool like Code Interpreter or Retrieval is enabled.
        
        // Create assistantResponse to .....
        // Response<Assistant> assistantResponse = await asst_client.CreateAssistantAsync(
        //     new AssistantCreationOptions("lil-MissKind-Luna")
        //     {
        //         Name = "Luna Law Firm Assistant",
        //         Instructions = $"""
        //         You are a lawyer's receptionist.
        //         You work for {companyname}.
        ...
        //     Assistant assistant = assistantResponse.Value;
            ...
            //Creating a thread, 
            // Response<AssistantThread> threadResponse = await asst_client.CreateThreadAsync();
            // AssistantThread thread = threadResponse.Value;

            // //With a thread created, messages can be created on it:
            // Response<ThreadMessage> messageResponse = await asst_client.CreateMessageAsync(
            //     thread.Id,
            //     MessageRole.Assistant,
            //     "I had a injury at a work place and need legal assistance"
            // );

            // // A run can then be started that evaluates the thread against an assistant:

            // Response<ThreadRun> runResponse = await asst_client.CreateRunAsync(
            //     thread.Id,
            //     new CreateRunOptions(assistant.Id)
            //     {
            //         AdditionalInstructions = "Please address user as Ayush"
            //     });
            ...
            //     runResponse = await asst_client.GetRunAsync(thread.Id, runResponse.Value.Id);
            // }
            ...
            // Response<PageableList<ThreadMessage>> afterRunMessagesResponse = await asst_client.GetMessagesAsync(thread.Id);
...
*/
        ...
        Console.WriteLine($"Postdates length: {posdates.Length}");
        string weekday= posdates[(int)DateTime.UtcNow.AddHours(Int32.Parse(timezone)).DayOfWeek];
        ...
        {
            // transcript = "Receptionist: Good Afternoon, thank you for calling Schreid and Hilton Law, my name is Luna, their AI receptionist. How can I help you? Caller: I want to set up an appointment.Receptionist: Of course! May I ask what type of legal matter you're calling about? We specialize in Criminal, Family, and Personal Injury Law. Caller: Yeah, personal injury law.Receptionist: Great! I'm here to help with that. Can you please share a bit about your personal injury case? What happened?"
            // + "Caller: Yeah, I was driving and a truck blew a red light, came out of nowhere and hit me. I suffered severe injuries and I had to get medical treatment for them. And that cost like $20,000 and my car was totaled. So that's at least $50,000 worth of damages.Receptionist: Thank you for sharing that important information. It sounds like a challenging situation. Could you please tell me your full name?"
            // + "Caller: Yeah, my full name is Power Ranger.Receptionist: Thank you, Power Ranger. That's quite the situation you have on your hands. Can I have your email address and phone number, please? This will help us reach you regarding your case."
            // + "Caller: Yeah, my e-mail address is yellowrangerschool@gmail.com and my phone number is 123-456-7890.Receptionist: Thank you for sharing that, Power Ranger. It sounds like you've been through a difficult experience. Would you like to set up a meeting with Ethan Hilton to discuss your case further? If so, what time would work best for you?"
            // + "Caller: Yeah, that sounds good.Receptionist: What time would you like to set up the meeting with Ethan Hilton?"
            // + "Caller: 2:00 PM on Monday.Receptionist: Thank you, Power Ranger! I will inform Ethan Hilton of your requested meeting at 2:00 PM on Monday and will confirm it via email.";
            ...
            chatCompletionsOptions = new ChatCompletionsOptions()
                {
                    Messages = {
                        new ChatMessage(ChatRole.System, """
                You need to determine the name, number, and email of the caller in the supplied transcript.
                Use the below format, replacing the text in brackets with the result. Do not include the brackets in the output: 
            ...
            chatCompletionsOptions = new ChatCompletionsOptions()
                {
                    Messages = {
                        new ChatMessage(ChatRole.System, """
                You need to determine the sentiment of the caller and helpfulness of the receptionist in the provided transcript. When finding the sentiment of the caller, do not take into account the situation/reason for call.
                Only look at how the caller feels about the receptionist and his experience with the firm in the provided transcript.
                Also determine how likely the caller will continue to work with the firm. 
                To determine this, you can use how close the scheduled appointment is to the current date. The closer the appointment the more likely the caller will continue to work with the firm.
                Use the below format, replacing the text in brackets with the result. Do not include the brackets in the output: 
                 Effectiveness of Receptionist:[The helpfulness and effectiveness of the receptionist on scale of 1-10. Take into account how consice, nice, professional, and non-repetetive the receptionist was.] out of 10

                 Sentiment of Caller:[The sentiment of the call on scale of 1-10] out of 10
                 Likelihood of Caller continuing with the firm:[The likelihood of the caller continuing with the firm on a scale of 1 to 10] out of 10
            ...
            var sentiment_score = await GetChatGPTResponse(transcript, chatCompletionsOptions);
            // Console.WriteLine(sentiment_score);

            chatCompletionsOptions = new ChatCompletionsOptions()
                {
                    Messages = {
                        new ChatMessage(ChatRole.System, """
                You need to determine priority of this case. Take into account urgency of the issue, money at stake, status of client, and other important factors considered by law firms when priortiziing their clients.
                These are some indicators of high priority cases: Significant financial damages or large settlement potential. Involvement of multiple parties or large corporations. Serious or permanent injuries resulting from the case. 
                Clear liability or evidence strongly supporting the claim. Significant future impact on the plaintiff's livelihood. Cases involving wrongful death or catastrophic injury. Extensive medical treatment or long-term care required. 
                High-profile nature or potential for media attention. If unsure, say low priority.

                Use the below format, replacing the text in brackets with the result. Do not include the brackets in the output: 
                 Priority:[The priority of the call on a scale of 1-10 (with 10 being the highest)] out of 10; [High, Medium, Low Priority]
                 Reason for Priority:[The reason for priority. Use some of the indicators above if applicable.]

            """),
                        },
                    MaxTokens = 1000
                };
            var priority = await GetChatGPTResponse(transcript, chatCompletionsOptions);

            chatCompletionsOptions = new ChatCompletionsOptions()
                {
                    Messages = {
                        new ChatMessage(ChatRole.System, """
                You need to determine if the caller gave their contact info. You also need to determine if they requested and scheduled a specific consulation towards the end of the call. 

                Use the below format, replacing the text in brackets with the result. Do not include the brackets in the output:
                Contact Info Received: [0 or 1 (1 means yes, 0 means no)]
                Consultation Requested: [0 or 1 (1 means yes, 0 means no)]

            """),
                        },
                    MaxTokens = 1000
                };
            var contact_recieved = await GetChatGPTResponse(transcript, chatCompletionsOptions);
            ...
            chatCompletionsOptions = new ChatCompletionsOptions()
                {
                    Messages = {
                        new ChatMessage(ChatRole.System, $"""
                The text passed to you is a transcription of a call between a receptionist and a caller.
                Determine whether it's likely the call is a spam, fishing, or generally unwanted call. If not, briefly state why it is a valid call. 
                Use the below format, replacing the text in brackets with the result.
                Spam Likelihood: [Whether the caller is suspicious or nefarious]
            """),
                        },
                    MaxTokens = 1000
                };
            ...
            //adding KPIs and transcript to email body and outputting it to console.
            
            // string KPIs = "\n\n KPIs: \n Summary length:" + summary.Length + "\n Transcript length:" +  transcript.Length + "\n Speedup Due to Luna: "+ (int)(100* (((double)transcript.Length/summary.Length)-1)) + "%" +
            string KPIs = "";
            KPIs += "\n\nKPIs: \n Call Length: "+(int)(transcript.Length/9.4) + " seconds";
            KPIs += "\n Time Saved for Firm: " + (int)((double)(transcript.Length-summary.Length)/9.4) + " seconds";
            KPIs += "\n Avg Response Time: "+ total_response_time/response_num + "ms";
            KPIs += "\n " + mspam.Groups[0].Value + "\n" + sentiment_score + "\n\n" + priority + "\n" + contact_recieved;
            // KPIs += "\n Total Response time: " + totalResponseTime;
            body += KPIs;

            string filePath = "kpis.txt";
            File.AppendAllText(filePath, KPIs);
...
// This code retrieves your connection string from an environment variable.
        ...
        client.GetEventProcessor().AttachOngoingEventProcessor<RecognizeCompleted>(answerCallResult.CallConnection.CallConnectionId, async (recognizeCompletedEvent) =>
        {
           ...
           /* client.GetCallRecording().StopAsync(recresponse.Value.RecordingId);
            ...
            {
                Console.WriteLine($"\nRecognized speech: {speech_result.Speech}");
                ...
                else if (transferme && (speech_result.Speech.ToLowerInvariant().Contains("no") || speech_result.Speech.ToLowerInvariant().Contains("don't") || speech_result.Speech.ToLowerInvariant().Contains("not"))) {
                            transferme=false;
                            Console.WriteLine($"Client doesnt want to transfer");
                }
                if ((can_transfer) && (!speech_result.Speech.ToLowerInvariant().Contains("no") && !speech_result.Speech.ToLowerInvariant().Contains("don't") && !speech_result.Speech.ToLowerInvariant().Contains("not") && !speech_result.Speech.ToLowerInvariant().Contains("wait") && ( (speech_result.Speech.ToLowerInvariant().Contains("transfer")  || speech_result.Speech.ToLowerInvariant().Contains("forwarded") ||speech_result.Speech.ToLowerInvariant().Contains("forward me") || (speech_result.Speech.ToLowerInvariant().Contains("talk to") || (speech_result.Speech.ToLowerInvariant().Contains("speak to")) && speech_result.Speech.ToLowerInvariant().Contains("someone else") && !speech_result.Speech.ToLowerInvariant().Contains("you"))))))
                {
                    ...
                    if (!transferme) {
                        ...
                        if (speech_result.Speech.ToLowerInvariant().Contains("no") || speech_result.Speech.ToLowerInvariant().Contains("don't") || speech_result.Speech.ToLowerInvariant().Contains("not")) {
                            transferme=false;
                            Console.WriteLine($"Client doesnt want to transfer");
                            var spoken = new TextSource(transferWarningPrompt)
                                {
                ...
                {                    
                    transcript+="\nCaller: "+speech_result.Speech;
                    
                    if (clientCategoryDetermined == ""){
                          var chatCompletionsClientCategory = new ChatCompletionsOptions()
                        {
                            Messages = {
                                new ChatMessage(ChatRole.System, $"""
                            You need to determine if the caller is a new lead or not. 
                            Use the below format, replacing the text in brackets with the result. Do not include the brackets in the output.
                            [0 or 1 (1 means yes, 0 means no)]
                        """),
                                },
                            MaxTokens = 80 // Arbitrary small number
                        };
                    clientCategoryDetermined = await GetChatGPTResponse(speech_result.Speech, chatCompletionsClientCategory);
                    Console.WriteLine($"GPT Response on client category: {clientCategoryDetermined}");    
                    }
                  
                    
                    
                    // To go into if conditions depening on the client type
                    if(clientCategoryDetermined.Contains("1")) {
        
                        var chatGPTResponse = await GetChatGPTResponse(speech_result.Speech,  chatCompletionsOptions);
                        Console.WriteLine($"Luna: {chatGPTResponse}");
                        ...
                        if (!disconnected) {
                            ...
                            watch.Stop();
                        response_time = (int)watch.ElapsedMilliseconds;
                        response_num+=1;
                        total_response_time += (int)response_time;
                        Console.WriteLine($"Response Time: {response_time}");
                        }
                    }
                    else {

                        var asstPromptWorkHrs = $"""
...
async Task HandleChatResponse(string chatResponse, CallMedia callConnectionMedia, string callerId,  string voice, string calllang, string context = "OpenAISample")
{
    ...
    <speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="https://www.w3.org/2001/mstts" xml:lang="en-US">
        <voice name="{voice}">
            <mstts:express-as style="assistant" styledegree="2">
                <prosody rate="1.2"> <!-- This increases the speaking rate by 20% --> 
    ...
    var recognizeOptions =
            ...
            targetParticipant: CommunicationIdentifier.FromRawId(callerId))
        {
            ...
            EndSilenceTimeout = TimeSpan.FromMilliseconds(1000) // how long the chatbot will wait for pauses before interrupting the caller. Default was 500, but I changed it to 1000. 
        };
...
async Task HandleRecognizeAsync(CallMedia callConnectionMedia, string callerId, string message, string voice, string calllang)
{
    ...
    var recognizeOptions =
            ...
            targetParticipant: CommunicationIdentifier.FromRawId(callerId))
        {
            ...
            SpeechLanguage = calllang,
            EndSilenceTimeout = TimeSpan.FromMilliseconds(1)
        };
...
async Task LanguageRecognizeAsync(CallMedia callConnectionMedia, string callerId, string[] languages)
{
   ...
   string inside="";
    ...
    var recognizeOptions = new CallMediaRecognizeDtmfOptions( CommunicationIdentifier.FromRawId(callerId), 1) {
        InitialSilenceTimeout = TimeSpan.FromSeconds(30),
        Prompt = greetingPlaySource,
        InterToneTimeout = TimeSpan.FromSeconds(1),
